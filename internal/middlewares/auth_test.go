...
func TestAuth(t *testing.T) {
	t.Run("Cookie Generation", func(t *testing.T) {
		t.Run(
	...
	t.Run("Token Verification", func(t *testing.T) {
		...
		t.Run(
			"Невалидный base64 в encodedUserID", func(t *testing.T) {
				...
				assert.Error(t, err)
				assert.Contains(t, err.Error(), "invalid encoded userID")
				assert.Equal(t, request, result)
			},
		)

		t.Run(
			"Проверка сообщения InvalidUserFormatError", func(t *testing.T) {
				invalidErr := &InvalidUserFormatError{message: "test message"}
				assert.Equal(t, "test message", invalidErr.Error())
			},
		)
		...
		t.Run(
			"Проверка типа ошибки InvalidUserFormatError", func(t *testing.T) {
				...
				result, err := verifyToken(request, invalidCookie)

				var invalidUserErr *InvalidUserFormatError
				require.Error(t, err)
				assert.True(t, errors.As(err, &invalidUserErr), "Ошибка должна быть типа InvalidUserFormatError")
				assert.Contains(t, invalidUserErr.message, "invalid encoded userID")
				assert.Equal(t, request, result)
			},
		)
	...
	t.Run("Middleware Integration", func(t *testing.T) {
		...
		t.Run(
			"Проверка HTTP кода для InvalidUserFormatError", func(t *testing.T) {
				...
				assert.False(t, nextHandler.called, "NextHandler НЕ должен быть вызван при неверном формате")
				assert.Equal(t, http.StatusUnauthorized, writer.Code)
				assert.Contains(t, writer.Body.String(), "invalid encoded userID")
			},
		)
